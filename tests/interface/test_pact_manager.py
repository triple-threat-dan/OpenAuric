...
async def test_handle_message_no_pending_task(manager):
    ...
    event = PactEvent(platform="telegram", sender_id="user1", content="hello", raw_event={})
...
async def test_handle_message_approval_with_adapter(manager):
    ...
    event = PactEvent(platform="discord", sender_id="user1", content="yes, go ahead", raw_event={})
    await manager.handle_message(event)
...
async def test_handle_message_approval_no_adapter(manager):
    ...
    event = PactEvent(platform="telegram", sender_id="user1", content="approve", raw_event={})
    await manager.handle_message(event)
...
async def test_handle_message_denial_with_adapter(manager):
    ...
    event = PactEvent(platform="telegram", sender_id="user1", content="stop that right now", raw_event={})
    await manager.handle_message(event)
...
async def test_handle_message_denial_no_adapter(manager):
    ...
    event = PactEvent(platform="discord", sender_id="user1", content="no", raw_event={})
    await manager.handle_message(event)
...
async def test_handle_message_pending_task_not_approval_or_denial(manager):
    ...
    event = PactEvent(platform="discord", sender_id="user1", content="what is the task about?", raw_event={})
    await manager.handle_message(event)
...
def test_get_all_tools_definitions(manager):
    ...
    mock_adapter3 = MagicMock() # Mock empty return
    mock_adapter3.get_tools_definition.return_value = ""
    
    manager.adapters["a"] = mock_adapter1
    manager.adapters["b"] = mock_adapter2
    manager.adapters["c"] = mock_adapter3
    
    defs = manager.get_all_tools_definitions()
    assert "Tool A definition" in defs
    assert "Tool B definition" in defs
    assert "Tool A definition\n\nTool B definition" == defs or "Tool B definition\n\nTool A definition" == defs


def test_get_tools_schema(manager):
    mock_adapter1 = MagicMock()
    mock_adapter1.get_tools_schema.return_value = [{"name": "tool_a"}]
    
    mock_adapter2 = MagicMock()
    mock_adapter2.get_tools_schema.return_value = [{"name": "tool_b"}]
...
def test_get_tool_names(manager):
    mock_adapter1 = MagicMock()
    mock_adapter1.get_tool_names.return_value = {"tool_a", "tool_b"}
    
    mock_adapter2 = MagicMock()
    mock_adapter2.get_tool_names.return_value = {"tool_c"}
...
async def test_execute_tool_intercept_discord_dm(manager):
    ...
    mock_adapter.execute_tool.assert_called_once_with("discord_send_dm", {"user_id": "999", "content": "Hello there"})
